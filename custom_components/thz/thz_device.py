...
class THZDevice:
    ...
    def escape(self, data: bytes) -> bytes:
        """Escape protocol-special bytes for sending.

        Per THZ protocol, before sending:
        - Each 0x10 byte must be doubled (0x10 -> 0x10 0x10)
        - Each 0x2B byte must be escaped (0x2B -> 0x2B 0x18)

        This is applied ONLY to checksum+command portion, NOT header/footer.

        Args:
            data: Data bytes to escape.

        Returns:
            Escaped data ready for transmission.
        """
        # Order matters: first handle 0x10, then 0x2B
        data = data.replace(DATALINKESCAPE, DATALINKESCAPE + DATALINKESCAPE)
        data = data.replace(b"\x2B", b"\x2B\x18")
        return data
    ...
    def construct_telegram(
        ...
        Per THZ protocol (matching Perl original):
        1. Checksum is calculated on unescaped data
        2. checksum + addr_bytes are escaped (0x10->0x1010, 0x2B->0x2B18)
        3. Final telegram = header + escaped(checksum + addr_bytes) + footer
        ...
        """
        # Combine checksum and command, then escape special bytes
        # This matches Perl: $checksumadded = checksum . $cmd, then THZ_replacebytes
        checksum_and_cmd = checksum + addr_bytes
        escaped = self.escape(checksum_and_cmd)
